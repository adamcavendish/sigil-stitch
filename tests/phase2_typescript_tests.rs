...
fn test_ts_interface() {
    ...
    fb2.add_param(ParameterSpec::new("entity", TypeName::primitive("T")));
    fb2.returns(TypeName::generic(
        TypeName::primitive("Promise"),
        vec![TypeName::primitive("void")],
    ));
    tb.add_method(fb2.build());
...
fn test_ts_abstract_class() {
    ...
    fb.is_abstract();
    fb.add_param(ParameterSpec::new("req", TypeName::primitive("Request")));
