...
fn test_ts_simple_annotation_on_fun() {
    ...
    let output = render_fun(&fb.build(), &ts, DeclarationContext::TopLevel);
...
fn test_ts_annotation_on_field() {
    ...
    let output = render_field(&fb.build(), &ts, DeclarationContext::Member);
...
fn test_rust_derive_annotation() {
    ...
    tb.visibility(Visibility::Public);
    let output = render_type(&tb.build(), &rs);
...
fn test_rust_annotation_on_field() {
    ...
    let output = render_field(&fb.build(), &rs, DeclarationContext::Member);
...
fn test_rust_annotation_on_enum_variant() {
    ...
    tb.add_variant(EnumVariantSpec::new("Custom"));
    let output = render_type(&tb.build(), &rs);
...
fn test_cpp_nodiscard_annotation() {
    ...
    let output = render_fun(&fb.build(), &cpp, DeclarationContext::TopLevel);
...
fn test_c_attribute_annotation() {
    ...
    let output = render_fun(&fb.build(), &c, DeclarationContext::TopLevel);
...
fn test_java_suppress_warnings() {
    ...
    let output = render_fun(&fb.build(), &java, DeclarationContext::TopLevel);
...
fn test_python_dataclass_decorator() {
    ...
    let output = render_type(&tb.build(), &py);
