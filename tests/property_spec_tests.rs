...
fn test_ts_getter_only() {
    ...
    let output = render_property(&pb.build(), &ts, DeclarationContext::Member);
...
fn test_js_getter_setter() {
    ...
    pb.getter(CodeBlock::of("return this._name", ()).unwrap());
    pb.setter("value", CodeBlock::of("this._name = value", ()).unwrap());
    let output = render_property(&pb.build(), &js, DeclarationContext::Member);
...
fn test_swift_getter_only() {
    ...
    let output = render_property(&pb.build(), &swift, DeclarationContext::Member);
...
fn test_kotlin_getter_only() {
    ...
    pb.getter(CodeBlock::of("return _count", ()).unwrap());
    let output = render_property(&pb.build(), &kt, DeclarationContext::Member);
