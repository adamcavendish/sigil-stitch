...
fn main() {
    ...
    tb.add_field(FieldSpec::builder("host", TypeName::primitive("str")).build());
    ...
    to_json.add_param(ParameterSpec::new("self", TypeName::primitive("")));
    ...
    tb.add_method(to_json.build());

    // Build a standalone function.
    let mut greet = FunSpec::<Python>::builder("greet");
    greet.add_param(ParameterSpec::new("name", TypeName::primitive("str")));
